...
#make copy of datset
hp_consumption3 <- hp_consumption

#create new DateTime variable of class POSIXct by combining Date and Time variables 
hp_consumption3$DateTime <- as.POSIXct(paste(hp_consumption3$Date, hp_consumption3$Time), format="%d/%m/%Y %H:%M:%S")
class(hp_consumption3$DateTime)
#[1] "POSIXct" "POSIXt"

head (hp_consumption3)

#subset dataset to date between "2007-02-01" and "2007-02-02"
hp_2daysconsumption <-subset(hp_consumption3, DateTime >= as.POSIXct('2007-02-01 00:00:00')) 
...
legend("topright",legend= c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c('grey','red','blue'), lty=1,lwd=1.5) 

#export plot to file of type png
dev.copy(png, file = "C:/coursera/data_scientist_training/figure/Plot3.png", width = 480, height = 480) 
dev.off()